...
function run_aamks($path, $user_id, $irange, $scenario_id){
    ...
    $command = $AAMKS_PATH."/env-server/bin/python3 ".$AAMKS_PATH."/slurm.py -t l -p ".$path." -u ".$user_id." -n ".$irange[0]." ".$irange[1]." -s".$scenario_id." 2>&1";
    $output = shell_exec($command);
    return $output;
}
function run_beck_anim($path, $project, $scenario, $iter){
    $AAMKS_PATH=getenv('AAMKS_PATH');
    $command = $AAMKS_PATH.'/env-server/bin/python3 '.$AAMKS_PATH.'/slurm.py -t a -p'.$path.' -r '.$project.' -s '.$scenario.' -i '.$iter." 2>&1";
    $output = shell_exec($command);
    return $output;
}
