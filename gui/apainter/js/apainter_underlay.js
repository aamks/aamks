...
function uimg_scale(floor) {//{{{
	tt=parse_transform(floor);
	ss=tt.scale * Number($("#uimg_scale").val()) / Number($("#p1").attr('width'));
	d3.select("#uimg"+floor).attr("transform","translate("+tt.x+","+tt.y+") scale("+ss+")");
	$("#p1").remove();
}
//}}}
...
function underlay_attribs(floor,aa=0) {//{{{
	if (aa==0) {
		...
		d3.select("#uimg"+floor).style("opacity", aa.opacity).attr('filter', null).attr("invert", 0).attr("type", aa.type);
		if(aa.invert==1) { uimg_single_attrib(floor , 'invert' , 1); }
		if("transform" in aa) { uimg_single_attrib(floor, 'transform', aa.transform); }
	}
}
//}}}
function underlay_zoomer(floor) {//{{{
	...
	d3.select("#underlay"+floor)
		.call(d3.zoom()
			.on("zoom", function() {
				var ss=parse_transform(floor)['scale'];
				d3.select("#uimg"+floor).attr("transform","translate("+Math.round(d3.event.transform.x)+","+Math.round(d3.event.transform.y)+") scale("+ss+")"); 
			})
		)
}
//}}}
function parse_transform(floor){//{{{
	arr=$("#uimg"+floor).css("transform").split(", ");
	t={'scale': Number(arr[3]), 'x': Number(arr[4]), 'y': Number(arr[5].split(")")[0]) };
	return t
}
//}}}
...
function uimg_add(e) {//{{{
	...
	reader.onload = function(event) {
		...
		if(['jpeg', 'png', 'pdf', 'svg'].indexOf(type) > -1) {
			uSetup={ 'floor': floor, 'type': type, 'base64': base64, 'opacity': 0.5, 'transform': "translate(0,0) scale(1)" };
