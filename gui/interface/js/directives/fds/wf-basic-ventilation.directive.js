angular.module('wf-basic-ventilation.directive', [ 'wf-globals.service','wf-safe-apply.service', 'ngLodash', 'wf-fds-object.service','wf-list.service', 'wf-dialog.service', 'wf-id-generator.service', 'katex'])
.directive('wfBasicVentilation', function( Globals, GlobalValues, $rootScope, Fds, Vent, SurfVent,  Ramp , lodash, $timeout, List, DialogManager, SafeApply, IdGenerator ) {
...
//}}}
		...
		scope.$on('scroll', function(event, data) {
			if(!isNaN(data.position)) {
				if(data.name=='surfvent') {

					scope.ui.ventilation.surf.scrollPosition=data.position;
				} else if(data.name=='vent'){
					scope.ui.ventilation.vent.scrollPosition=data.position;
				}
			}
		})
//}}}
		...
		scope.$on('geometry_update', function(event, data) {
			...
			if(scope.vent && scope.currentVentList.type=='file') {
				...
				var vent_index=lodash.findIndex(scope.ventilation.vents,function(vent) {
				...
				if(vent_index>=0) {
					...
					scope.ui.ventilation.vent.begin=begin_index;
			...
			lists.ventList=new List(scope, 'vent', scope.ventilation.vents, null, scope.ui.ventilation.vent, scope.indexes.vent, scope.listRange, Vent, 'vent', 'file');
		...
		scope.$on('geometry_select', function(event, data) {
			scope.ui.ventilation.vent.begin=data.begin;
...
//}}}
		...
		scope.functions.surf={
			switch_lib: function() {
				if(scope.ui.ventilation.surf.lib=='closed') {
					scope.ui.ventilation.surf.lib='open';
				} else {
					scope.ui.ventilation.surf.lib='closed';
				}
		
			},
			switch_help: function() {
				if(scope.ui.ventilation.surf.help=='closed') {
					scope.ui.ventilation.surf.help='open';
				} else {
					scope.ui.ventilation.surf.help='closed';
				}
			...
			addNewRamp:function(surf) {
				...
				var surf_id = surf.id;
				var surf_index=lodash.findIndex(scope.ventilation.surfs,function(surface) {
					return surface.id==surf_id;
				})
				scope.ventilation.surfs[surf_index].ramp=tempRamp;
			},
...
//}}}
		...
		scope.functions.libSurf={
			...
			importItem:function(index) {
				var updatedIndex=index+scope.ui.ventilation.libSurf.begin;
				var base=scope.lib.importers.surfvent_import(scope.lib.ventsurfs[updatedIndex], scope.ventilation.surfs, scope.ramps.ramps, scope.lib.ramps);
...
//}}}
	...
	return {
		templateUrl: Globals.partialsUrl+'/fds/wf-basic-ventilation',
		link : link,
