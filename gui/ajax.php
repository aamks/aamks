...
function ajaxLaunchSimulation() { #{{{
	$demo=getenv("AAMKS_DEMO");
	if ($demo){
		//check if user is allowed to run
		...
		if (empty($r) OR $r[0]['valid_to'] < date("Y-m-d H:i:s")) {
			echo json_encode(array("msg"=>"This is a demo version - you need to be authorized to run calculations. Please contact <a class=blink href=mailto:aamks@apoz.edu.pl>aamks@apoz.edu.pl</a>", "err"=>1, "data"=>''));
			return;
		}
	}
