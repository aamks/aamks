<?php

$redis = new Redis();
$redis->connect(getenv("AAMKS_SERVER"), 6379);
$redis->auth(getenv("AAMKS_REDIS_PASS"));

function push($mess) {
    global $redis;
    $redis->lpush('aamks_queue', json_encode($mess));
}
function pop() {
    global $redis;
    return $redis->brpop('aamks_queue');
}
function show(){
    global $redis;
    return $redis->lrange('aamks_queue', 0, -1);
}
function len(){
    global $redis;
    return $redis->lLen('aamks_queue');
}
function gen_id(){
    $now = DateTime::createFromFormat('U.u', microtime(true));
    return $now->format("Ymd-H:i:s.u");
}
function message(){
    $message = array(
        "id" => gen_id(), #20231113-15:00:00.000000
        "AA" => array( 
            "PROJECT" => getenv("AAMKS_PROJECT"),
            "PATH" => getenv("AAMKS_PATH"),
            "SERVER" => getenv("AAMKS_SERVER"),
            "PG_PASS" => getenv("AAMKS_PG_PASS")),
        "data" => ''
        );
    return $message;
}
function run_aamks($path){
    $mess = message();
    $mess["data"] = array("aamks" => $path);
    push($mess);
}
function run_beck_anim($path){
    $mess = message();
    $mess["data"] = array("anim" => $path);
    push($mess);
}
function run_beck_new($path, $scenarios = ""){
    $mess = message();
    $mess["data"] = array("results" => [$path, $scenarios]);
    push($mess);
}

?>