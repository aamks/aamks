...
update() { #{{{
	...
	rm -rf /etc/aamksconf.json
	hostname
	echo "{ \"AAMKS_SERVER\": \"$_AAMKS_SERVER\" }"  | tee /etc/aamksconf.json
	echo 
}
#}}}
install() { #{{{
	...
	apt-get update
	apt-get --yes install git python3-pip xdg-utils unzip cmake gearman ipython3 python3-urllib3 libboost-python-dev libgfortran5
	pip3 install --upgrade pip
	pip3 install shapely scipy numpy Cython
	...
	mv aamks $_AAMKS_PATH
	chown -R $USER:$USER $_AAMKS_PATH
...
exit; }
	...
	mv ~/go/bin/recast /usr/local/bin
	...
	mkdir /home/aamks_users
	chmod 777 /home/aamks_users
}
#}}}
print_help() { #{{{
	cat << EOF
