...
class CFASTRoomsChoice: 
    def __init__(self, number,sim_id):# {{{

        self.project = os.environ['AAMKS_PROJECT']
        self.sim_id = sim_id
        new_sql_path = os.path.join(os.environ['AAMKS_PROJECT'], "workers", f"{self.sim_id}", f"aamks_{self.sim_id}.sqlite")
        self.s=Sqlite(new_sql_path)

        self.doors = []
        self.rooms = []
        self.rooms_number = number
    ...
    def dijkstra(self, graph, start_door_name):
        ...
        return distances
    def get_closest_rooms(self, room_in_fire_id):
            ...
            if len(room['doors']) > 1:
                for i in range(len(room['doors'])):
                    for j in range(i + 1, len(room['doors'])):
                        ...
                        # kubatura pomieszczenia jest wagą bo dym musi ją wypełnić
                                        
                        door_dist = room['width']*room['length']*room['height']
        ...
        # wczesniej odleglosc nie pitagoras tylko kubatura pomieszczenia przejsciowego!!!!!
        #15 bierzemy
        #połazone holem traktuj jak 1 pomieszczenie
        #pozniej jak chce wiecej z animatora to 45
        #pozmniej jeszcze wiecej to 99
    ...
    def print_rooms(self):
        ...
        # Wyciąganie self.rooms_number pomieszczeń z najmniejszymi wartościami distance
        closests_rooms = sorted_finite_rooms[:self.rooms_number]
        ...
        for floor in floors:
            ...
            for room in self.rooms:
                if room['floor'] == floor:
                    # Sprawdzamy, czy pomieszczenie należy do self.rooms_number najmniejszych wartości distance
                    is_smallest = room in closests_rooms
