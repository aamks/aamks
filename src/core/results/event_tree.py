...
class EventTreeFED:
    ...
    def draw_tree(self):
        ...
        ax.text(0.5 * 1.54, 0.5 * 1.7, 'Scenariusz\n pożarowy', horizontalalignment='center', verticalalignment='center',
        ...
        c = patches.Ellipse(xy=(0.673, 0.442), width=0.02, height=0.04, fill=True, transform=ax.transAxes, clip_on=False, color='b')
        ax.add_patch(c)
        p1 = float(self.p_general) * self.p_develop * self.p_dcbe * self.p_fed_f
        ax.text(4.65, -0.335, r'$P_4 =  %.2E; S_4 = H$' % Decimal(p1))

        c = patches.Ellipse(xy=(0.673, 0.347), width=0.02, height=0.04, fill=True, transform=ax.transAxes, clip_on=False, color='b')
        ax.add_patch(c)
        p2 = float(self.p_general) * self.p_develop * self.p_dcbe * self.p_fed_m
        ax.text(4.65, -0.2, r'$P_3 =  %.2E; S_3 = M$' % Decimal(p2))

        c = patches.Ellipse(xy=(0.673, 0.25), width=0.02, height=0.04, fill=True, transform=ax.transAxes, clip_on=False, color='b')
        ax.add_patch(c)
        p = float(self.p_general) * self.p_develop * self.p_dcbe * self.p_fed_l
        ax.text(4.65, -0.052, r'$P_2 =  %.2E; S_2 = L$' % Decimal(p))

        c = patches.Ellipse(xy=(0.673, 0.159), width=0.02, height=0.04, fill=True, transform=ax.transAxes, clip_on=False, color='b')
        ax.add_patch(c)
...
class EventTreeSteel:
    ...
    def draw_tree(self):
        ...
        head = ['Inicjacja\n pożaru', 'Ugaszony \nw zarodku', r'$T_k < 450$', r'$t_k < t_{WCBE}$']
        ...
        ax.text(3.5, 0.135, 'NIE')
        ax.text(3.5, 0.065, p_time[2])

        fed_l = [[3.3, 4.4], [-0.325, -0.325], 'P = {}'.format(self.p_time_less)]
        ax.plot(fed_l[0], fed_l[1], linewidth=2, color = 'b')
        ax.text(3.5, -0.305, 'TAK')
        ax.text(3.5, -0.375, fed_l[2])

        ax.plot([3.3, 6.5], [-0.5, -0.5], linewidth=0.1, color='white')

        c = patches.Ellipse(xy=(0.673, 0.565), width=0.02, height=0.04, fill=True, transform=ax.transAxes, clip_on=False, color='b')
        ax.add_patch(c)
